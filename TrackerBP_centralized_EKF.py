...
class TrackerBP_EKF:
    ...
    def compute_beta(self,
        ...
        # Measurement gating for centralized EKF: only targets within sensing range
        self.valid_target_indices = [i for i, mean in enumerate(self.gamma.mean)
                                     if np.linalg.norm(mean[:2] - sensor_pos) <= self.sensingRange]
        ...
        for m in range(num_measurements):
            ...
            for idx, target_idx in enumerate(self.valid_target_indices):
                ...
                # Mahalanobis distance for gating if needed, here we compute likelihood
                ...
                try:
                    ...
                    self.beta[m + 1, idx] = 0 # Assign low likelihood if S is singular
    ...
    def compute_kappa_iota(self):
        ...
        if num_targets == 0 or num_measurements == 0:
            ...
            if self.iota.ndim == 1:
                 self.iota = self.iota / (1 + self.iota) # Simplified for no targets
            return
    ...
    def compute_gamma(self):
        ...
        for idx, target_idx in enumerate(self.valid_target_indices):
            ...
            # Weights for each hypothesis (missed detection + measurements)
    ...
    def init_new_track_from_measurement(self, measurement: np.ndarray, sensor_pos: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        # Initial covariance: prefer explicit parameter if provided, else derive from measurement noise
        if self.init_cov is not None:
            ...
            cov = np.zeros((4, 4))
            cov[:2, :2] = R_cartesian
            cov[2:, 2:] = self.velocity_noise_cov # Initial velocity uncertainty
