...
class TrackerBP:
    ...
    def compute_alpha(self, sensor_index) -> None:
        """
        ...
        for target in range(num_targets):
            ...
            # Update existence using survival probability only (no range gating)
            self.alpha.existence[target] = self.p_s * deepcopy(self.gamma.existence[target])
        # Propagate alpha mixture back to gamma
