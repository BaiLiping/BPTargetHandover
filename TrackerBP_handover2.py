...
class TrackerBP:
    ...
    def compute_alpha(self, handover_index, base_station_index, threshold) -> Any:
        ...
        # Propagate predicted mixture to gamma
    ...
    def compute_gamma(self, handover_index: int = None, threshold: float = 0.5) -> BernoulliMixture:
        """
        ...
        for target in range(num_targets):
            ...
            # for each target determine of handover is needed
            particles = self.gamma.states[:2, :, target]                   # shape (2, num_particles)
            ...
            ratio = num_particles_in_range / self.num_particles
            if ratio > threshold and self.gamma.existence[target] > 0.95:
                self.handover_gamma_indices.append(target)                    
        # handover the posterior based on self.handover_alpha_indices
        num_handover = len(self.handover_gamma_indices)
        ...
        mask = np.ones(self.gamma.states.shape[2], dtype=bool)
        for idx, h_index in enumerate(self.handover_gamma_indices):
            new_index = h_index - idx
            new_handover.states[:, :, idx] = deepcopy(self.gamma.states[:, :, new_index])
            new_handover.existence[idx] = deepcopy(self.gamma.existence[new_index])
            new_handover.label[idx] = deepcopy(self.gamma.label[new_index])
            mask[new_index] = False  # Mark the index to exclude

        # Apply the mask to keep only the desired elements
        self.gamma.states = self.gamma.states[:, :, mask]
        self.gamma.existence = [self.gamma.existence[i] for i in range(len(mask)) if mask[i]]
        self.gamma.label = [self.gamma.label[i] for i in range(len(mask)) if mask[i]]
        ...
        new_existences = new_existences.tolist()  # Ensure it's a list
        ...
        self.gamma.existence = list(merged_existences)  # Ensure it's a list
    
        # Merge labels.
        if not self.gamma.label:
            merged_labels = self.varsigma.label
        else:
            # the plus here means merge
