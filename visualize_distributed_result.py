...
def visualize_distributed_result(results_folder, output_folder, trial_num, data):
    """
    Generate and save frames for one distributed tracking trial:
    ...
    frame_dir = os.path.join(output_folder, f"distributed_trial_{trial_num:04d}")
    ...
    for t in range(n_steps):
        ...
        # Save frame
        fig.savefig(os.path.join(frame_dir, f"distributed_trial_{t:04d}.png"), dpi=150)
    plt.close(fig)


if __name__ == '__main__':
    results_folder = '/Users/lipingb/Desktop/distributed_experiment'
    output_folder  = '/Users/lipingb/Desktop/Target_Handover'
    ...
    for trial in trial_files:
        ...
        with open(os.path.join(results_folder, trial), 'rb') as f:
            data = pickle.load(f)
        visualize_distributed_result(results_folder, output_folder, num, data)

    # Step 2: build GIF for one random trial only
    random_trial = random.choice(trial_nums)
    frames_path  = os.path.join(output_folder, f"distributed_trial_{random_trial:04d}")
    ...
    gif_path = os.path.join(output_folder, f"Distributed.gif")
