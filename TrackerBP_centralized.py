...
class BernoulliMixture:
    ...
    Now, existence and label are stored as lists.
    """
    def __init__(self,
...
class TrackerBP:
    ...
    def compute_alpha(self) -> None:
        ...
        for target in range(num_targets):
            ...
            # Update existence stored as a list.
    ...
    def compute_xi_sigma(self,
        ...
        # Initialize label and existence as lists for each measurement.
        self.varsigma.label = [0] * num_measurements
        self.varsigma.existence = [0] * num_measurements
        ...
        for meas in range(num_measurements):
            self.varsigma.states[:, :, meas] = self.sample_from_likelihood(sensor_measurements[:, meas],
                                                                            sensor_position)
            current_max_label = max(self.gamma.label) if len(self.gamma.label) > 0 else 0
            self.varsigma.label[meas] = current_max_label + meas + 1
            self.varsigma.existence[meas] = (measurements_likelihood[meas] *
                                             (self.mu_n * detection_probability) / clutter_intensity)
            self.xi[meas] = 1 + deepcopy(self.varsigma.existence[meas])
    ...
    def compute_beta(self,
        ...
        num_measurements = sensor_measurements.shape[1] if sensor_measurements.size else 0
        self.valid_target_indices = []
        num_targets_all = self.gamma.states.shape[2] if self.gamma.states.size else 0
        sensing_range = self.sensingRange
        predicted_range_valid = []
        predicted_bearing_valid = []
        for target_index in range(num_targets_all):
            # Compute predicted range and bearing for each particle.
            diff_x = self.gamma.states[0, :, target_index] - sensor_positions[0, sensor_index]
            diff_y = self.gamma.states[1, :, target_index] - sensor_positions[1, sensor_index]
            predicted_range = np.sqrt(diff_x ** 2 + diff_y ** 2)
            predicted_bearing = np.degrees(np.arctan2(diff_y, diff_x))
            predicted_bearing_valid.append(predicted_bearing)
            predicted_range_valid.append(predicted_range)
            if np.mean(predicted_range) <= sensing_range:
                self.valid_target_indices.append(target_index)
        ...
        if num_targets > 0:
            ...
            # Convert existence values for valid targets to a NumPy array.
            existence_array = np.array([self.gamma.existence[i] for i in self.valid_target_indices])
            existence = np.tile(existence_array, (num_measurements + 1, 1))

            for target_idx, target in enumerate(self.valid_target_indices):
                predicted_range = predicted_range_valid[target]
                predicted_bearing = predicted_bearing_valid[target]

                for m in range(num_measurements):
                    bearing_error = self.wrap_to_180(sensor_measurements[1, m] - predicted_bearing)
                    # Compute likelihood for each particle.
                    self.likelihood_table[m + 1, target_idx, :] = (
    ...
    def compute_gamma(self):
        ...
        for valid_target_idx, valid_target in enumerate(self.valid_target_indices):
            # missed detection
            weights = deepcopy(self.likelihood_table[0, valid_target_idx, :])
            # sum all the other measurements
            for m in range(num_measurements):
                weights += self.kappa[m, valid_target_idx] * self.likelihood_table[m+1, valid_target_idx, :]
            # sum of the weights of all particles
            sum_weights = np.sum(weights)
            isAlive = self.gamma.existence[valid_target] * sum_weights / num_particles
            isDead = 1 - self.gamma.existence[valid_target]
            self.gamma.existence[valid_target] = isAlive / (isAlive + isDead + 1e-12)
            if sum_weights > 0:
                norm_weights = weights / sum_weights
                indices = TrackerBP.resample(norm_weights, num_particles)
            self.gamma.states[:, :, valid_target] = deepcopy(self.gamma.states[:, indices, valid_target])
        ...
        # Merge labels using list concatenation.
        if len(self.gamma.label) == 0:
    ...
    def prune(self):
        ...
        if self.gamma.size() > 0:
            # Create a boolean list for valid existence probabilities.
            valid_idx = [e >= self.pruning_threshold for e in self.gamma.existence]
            if any(valid_idx):
                self.gamma.states = self.gamma.states[:, :, valid_idx]
                self.gamma.label = [lab for lab, valid in zip(self.gamma.label, valid_idx) if valid]
                self.gamma.existence = [e for e, valid in zip(self.gamma.existence, valid_idx) if valid]
            else:
    ...
    def calculate_likelihood_for_new_measurements(self, sensor_position: np.ndarray, sensor_measurements: np.ndarray) -> np.ndarray:
        ...
        # Compute differences (should result in numeric arrays)
