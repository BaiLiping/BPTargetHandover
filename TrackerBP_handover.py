...
class BernoulliMixture:
    ...
    def __init__(self,
        ...
        self.states = states
        # Convert existence to a list if it is a NumPy array or if None, set to empty list.
        if existence is None:
            self.existence = []
        elif isinstance(existence, np.ndarray):
            self.existence = existence.tolist()
        else:
    ...
    def combine(self, other: "BernoulliMixture") -> None:
        """
        Combine the current Bernoulli mixture with another mixture by concatenating
        the Bernoulli components.
        
        This operation concatenates:
            - `states` along the 3rd axis,
            - `existence` by list concatenation,
            - `label` by list concatenation.
        
        Raises:
            ValueError: If the state dimensions (first two axes) of the two mixtures do not match.
        """
        # If the other mixture is empty, nothing to combine.
        if other.states is None or other.states.size == 0:
            return
        
        # If current mixture is empty, just copy from other.
        if self.states is None or self.states.size == 0:
            self.copy_from(other)
        else:
            # Ensure state dimensions (first two axes) match
            if self.states.shape[:2] != other.states.shape[:2]:
                raise ValueError("State shapes of the mixtures do not match.")
            
            self.states = np.concatenate((self.states, other.states), axis=2)
...
class TrackerBP:
    def __init__(self, parameters: dict) -> None:
        ...
        sigma_v = 0.05   
        Q1 = np.array([[self.d_t ** 4 / 4, self.d_t ** 3 / 2], [self.d_t ** 3 / 2, self.d_t ** 2]], dtype=np.float64)
        Q = np.zeros((4, 4), dtype=np.float64)
        Q[np.ix_([0, 2], [0, 2])] = Q1
        Q[np.ix_([1, 3], [1, 3])] = Q1
        self.V = sigma_v ** 2 * Q  # Covariance of process noise.
        # Initialize Bernoulli mixtures.
        ...
        # a tag for track index to be handed over
        self.handover_indices = []
        self.handover_labels = []
        self.handover_alpha_indices = []

    def compute_alpha(self, handover_index, base_station_index, threshold) -> Any:
        """
        Predicts the target states (alpha) from the gamma mixture.
        """
        num_total_targets = self.gamma.states.shape[2]
        self.alpha.copy_from(self.gamma)
        handover_sensor_position = self.sensor_positions[:, handover_index]
        base_station_position = self.sensor_positions[:, base_station_index]
        self.handover_indices = []
        self.received_handover_indices = []

        self.handover_alpha_indices = []
    
        for target in range(num_total_targets):
            # Process noise: sample noise per particle and add to predicted states.
            ...
            self.alpha.existence[target] = self.p_s * self.alpha.existence[target]
            # Do not gate existence by range; handover decisions use particle ratio below
            # for each target
            particles = self.alpha.states[:2, :, target]                   # shape (2, num_particles)
            diffs     = particles - handover_sensor_position[:, None]      # broadcast to (2, num_particles)
            sq_dists  = np.sum(diffs * diffs, axis=0)                       # squared‐distance of each particle

            # mask = True for every particle whose distance < sensingRange
            mask      = sq_dists < (self.sensingRange ** 2)                 # shape (num_particles,)
            # sum of the mask is the number of particles within sensingRange
            num_particles_in_range = np.sum(mask)                           # scalar
            # proportion of particles within sensingRange
            ratio = self.p_d*num_particles_in_range / self.num_particles
            #if self.alpha.existence[target] * ratio > threshold and self.alpha.existence[target] > 0.5:
            if self.alpha.existence[target] * ratio > threshold:
                self.handover_alpha_indices.append(target)                    
                if self.alpha.label[target] not in self.handover_labels:
                    self.handover_labels.append(self.alpha.label[target])
                    self.handover_indices.append(target)
                   
        
        num_handover = len(self.handover_indices)
        new_handover = BernoulliMixture(
            states=np.zeros((4, self.num_particles, num_handover)),
            existence=[None] * num_handover,
            label=[None] * num_handover
        )
        for idx, h_index in enumerate(self.handover_indices):
            new_handover.states[:, :, idx] = self.alpha.states[:, :, h_index]
            new_handover.existence[idx] = self.alpha.existence[h_index]
            new_handover.label[idx] = self.alpha.label[h_index]
        return new_handover
    def incorporate_handover(self, handover: BernoulliMixture, handover_index: int) -> None:
        """
        Incorporates handover data into the current state.
        """
   

        if handover.existence and len(handover.existence) > 0:
            self.alpha.states = np.concatenate((self.alpha.states, handover.states), axis=2)
            self.alpha.existence.extend(handover.existence)
            labels = np.atleast_1d(self.alpha.label)
            current_max_label = labels.max() if labels.size else 0
            # add teh labels to received handover indices
            for meas in range(len(handover.existence)):
                new_label = current_max_label + meas + 1
                self.alpha.label.append(new_label)
                self.received_handover_indices.append(new_label)
                self.handover_labels.append(new_label)
    ...
    def compute_xi_sigma(self,
        ...
        # Allocate space for varsigma mixture states; for label and existence, use lists.
        ...
        for meas in range(num_measurements):
            self.varsigma.states[:, :, meas] = self.prior_new(sensor_measurements[:, meas],
                                                                            sensor_position)

            labels = np.atleast_1d(self.gamma.label)
            current_max_label = labels.max() if labels.size else 0
            self.varsigma.label.append(current_max_label + meas + 1)
            existence_val = (measurements_likelihood[meas] *
                             (self.mu_n * detection_probability) / clutter_intensity)
            self.varsigma.existence.append(existence_val)
            self.xi[meas] = 1 + deepcopy(existence_val)

    def compute_beta_handover_measurements(self,
                     sensor_measurements: np.ndarray,
                     sensor_index: int) -> None:
        """
        ...
        if num_targets > 0:
            ...
            v_factors[0, :] = 1
            # Convert gamma.existence (a list) to a NumPy array for broadcasting.
            existence_array = np.array(self.gamma.existence)
            existence_tile = np.tile(existence_array, (num_measurements + 1, 1))
            self.beta = existence_tile * np.mean(self.likelihood_table, axis=2) + (1 - existence_tile) * v_factors
            # the likelihood_collapsed is the sum of all the particles devided by the number of particles
            # for each measurement and target
            # the dimension of likelihood_collapsed is (num_measurements + 1, num_targets)
            likelihood_collapsed = np.sum(self.likelihood_table, axis=2) 

            #initiate a vector in the same format as sensor measurements
            # to store the measurement with the maximum likelihood
            #num_handover = len(self.handover_alpha_indices)
            #max_likelihood_measurements = np.zeros((2, num_handover))
            temp = []
            for idx, alpha_index in enumerate(self.handover_alpha_indices):
                # for target with index alpha_index
                # find out in the likelihood_collapsed table
                # the index of the measurement with the maximum likelihood
                # for the target with index alpha_index
                max_likelihood_index = int(np.argmax(likelihood_collapsed[1:, alpha_index]))
                if max_likelihood_index>0:
                    max_likelihood_measurement = sensor_measurements[:, max_likelihood_index-1]
                    temp.append(max_likelihood_measurement)
                # store maximum likelihood measurement to maximum_likelihood_measurements
            num_handover = len(temp)
            max_likelihood_measurements = np.zeros((2, num_handover))
            for idx, max_likelihood_measurement in enumerate(temp):
                max_likelihood_measurements[:, idx] = max_likelihood_measurement
        else:
            max_likelihood_measurements = np.zeros((2, 0))
        return max_likelihood_measurements
    ...
    def compute_gamma(self):
        ...
        for target in range(num_targets):
            ...
            # sum all the other measurements
            for m in range(num_measurements):
                weights += self.kappa[m, target] * self.likelihood_table[m+1, target, :]
            sum_weights = np.sum(weights)
            mean_weights = sum_weights / num_particles
            oneone = self.gamma.existence[target] * mean_weights
            onezero = 1 - self.gamma.existence[target]
            self.gamma.existence[target] = oneone / (oneone + onezero + 1e-12)
            if sum_weights > 0:
        ...
        # Update existence for new tracks using iota.
        # Convert varsigma.existence (a list) to an array for computation.
        varsigma_exist_array = np.array(self.varsigma.existence)
        new_existences = (self.iota * varsigma_exist_array) / (self.iota * varsigma_exist_array + 1 + 1e-12)
        new_existences = new_existences.tolist()
        if not self.gamma.existence:
            merged_existences = new_existences
        else:
            merged_existences = self.gamma.existence + new_existences
    ...
    def prune(self):
        ...
        if self.gamma.size() > 0:
            valid_idx = [i for i, exist in enumerate(self.gamma.existence) if exist >= self.pruning_threshold]
            if valid_idx:
                self.gamma.states = self.gamma.states[:, :, valid_idx]
                self.gamma.label = [self.gamma.label[i] for i in valid_idx]
                self.gamma.existence = [self.gamma.existence[i] for i in valid_idx]
            else:
    ...
    def estimate_state(self):
        ...
        if detected_states:
            ...
            estimates['label'] = detected_labels  # list of labels
    ...
    def estimate_cardinality(self):
        ...
        if self.gamma.existence:
            estimated_cardinality = sum(self.gamma.existence)
        else:
    ...
    def calculate_likelihood_for_new_measurements(self, sensor_position: np.ndarray, sensor_measurements: np.ndarray) -> np.ndarray:
        ...
        uniform_weight = 1 / (2 * np.pi * measurement_range**2)
    ...
    def prior_new(self, sensor_measurement: np.ndarray, sensor_position: np.ndarray) -> np.ndarray:
        """
